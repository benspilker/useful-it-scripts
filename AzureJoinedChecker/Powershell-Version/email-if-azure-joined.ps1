...
# Run dsregcmd and check join status
$dsregStatus = dsregcmd /status
$joinedLine = ($dsregStatus | Where-Object { $_ -match '^\s*IsDeviceJoined' }) -replace '\s+', ''
# At this point, $joinedLine should look like: "IsDeviceJoined:NO"
...
if ($joinedLine -eq 'IsDeviceJoined:NO') {
    Write-Output 'PC not AzureJoined. Sending Email.'
    $SMTPMessage = New-Object System.Net.Mail.MailMessage($EmailFrom, $EmailTo, $Subject, $Body)
    $SMTPClient.Send($SMTPMessage)
} else {
    Write-Output "PC is Azure AD joined. No email sent."
}
