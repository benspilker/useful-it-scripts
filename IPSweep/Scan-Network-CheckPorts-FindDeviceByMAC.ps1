...
for ($i = $Start; $i -le $End; $i++) {
    ...
    try {
        ...
        if ($PingResult) {
            ...
            # === Port Check for 443 (HTTPS) and 80 (HTTP) ===
            $Port80Open = Test-NetConnection -ComputerName $IP -Port 80 -WarningAction SilentlyContinue
            $Port443Open = Test-NetConnection -ComputerName $IP -Port 443 -WarningAction SilentlyContinue

            if ($mac) {
                Write-Host "$IP - True - MAC: $mac - Hostname: $Hostname - Ping: ${PingTime}ms"
                Write-Host "    HTTP (Port 80): $($Port80Open.TcpTestSucceeded)"
                Write-Host "    HTTPS (Port 443): $($Port443Open.TcpTestSucceeded)"
